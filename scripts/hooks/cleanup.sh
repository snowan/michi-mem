...
RETENTION_DAYS=30
if [ -f "$CONFIG" ]; then
  CONFIGURED=$(grep -o '"diary_retention_days":[[:space:]]*[0-9]*' "$CONFIG" | grep -o '[0-9]*')
...
if [ -d "$DIARY_DIR" ]; then
  DELETED=$(find "$DIARY_DIR" -name "*.md" -mtime +$RETENTION_DAYS -delete -print 2>/dev/null | wc -l | tr -d ' ')
